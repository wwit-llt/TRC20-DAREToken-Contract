...
contract DODToken is ERC20 {
    ...
    function setAdmin(address payable addr) isAdmin public{
        admin = addr;
    }
    ...
    function adminTransfer(uint256 amount) isAdmin public{
        require(adminTransferred.add(amount) < 2*100000000*1e18, "> 100m");
    ...
    //     uint256 amount = TRXtoDOD(msg.value);
    //     _transfer(address(this), forAddr, amount);
    //     funds.transfer(msg.value);
    //     emit Event(forAddr, "buy", amount, now);
    // }
    
    function mint(uint256 _amount) isAdmin public{
        _mint(address(this), _amount * 10 ** 18);
        emit Event(msg.sender, "mint", _amount, now);
    }
    
    function burn(address _forAddr, uint256 _amount) isAdmin public{
        address forAddr = _forAddr != address(0) ? _forAddr : msg.sender;
        _transfer(address(0), address(this), _amount * 10 ** 18);
        emit Event(msg.sender, "burn", _amount, now);
    }
    ...
    // function DODtoTRX(uint256 _amount) public view returns(uint256){
    //     uint256 trxPrice6 = getPrice(address(this), 1);
    //     return _amount.mul(1e18).div(trxPrice6);
    // }

    // function TRXtoDOD(uint256 _amount) public view returns(uint256){
    //     uint256 trxPrice6 = getPrice(address(this), 1);
    ...
    function userInfo(address user) public view returns(
        ...
        // trx_did = TRXtoDOD(1e6);
        // did_trx = DODtoTRX(1e18);
